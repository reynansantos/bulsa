...
// Mini sheet to log a partial payment
function PaymentSheet({ utang, onSave, onClose }) {
  ...
  const save = () => {
    if (!amt || +amt <= 0) return;
    const payment = { id: uid(), amount: +amt, date, note: note.trim(),
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginTop:8 }}>
            {[100,200,500,1000].filter(q=>q<=remaining+1).concat(remaining!==1000&&remaining<=5000?[Math.round(remaining)]:[] ).slice(0,5).map(q=>(
              <button key={q} onClick={()=>setAmt(String(q))} className="tap-btn"
                style={{ background:amt===String(q)?`${color}20`:C.card, border:`1px solid ${amt===String(q)?color+"55":C.border}`,
                  color:amt===String(q)?color:C.textSub, borderRadius:99, padding:"5px 12px", cursor:"pointer", fontSize:12, fontWeight:700, fontFamily:"DM Sans,sans-serif" }}>
                ₱{q.toLocaleString()}
              </button>
            ))}
            <button onClick={()=>setAmt(String(Math.round(remaining)))} className="tap-btn"
              style={{ background:amt===String(Math.round(remaining))?`${color}20`:C.card, border:`1px solid ${amt===String(Math.round(remaining))?color+"55":C.border}`,
                color:amt===String(Math.round(remaining))?color:C.textSub, borderRadius:99, padding:"5px 12px", cursor:"pointer", fontSize:12, fontWeight:800, fontFamily:"DM Sans,sans-serif" }}>
              Full ({fmt(remaining)})
            </button>
          </div>
          {/* After preview */}
          {+amt>0&&(
            <p style={{ margin:"8px 0 0", fontSize:11, fontFamily:"DM Sans,sans-serif",
              color: remaining - +amt <= 0 ? C.green : C.textSub }}>
              After: <strong style={{ color: remaining-+amt<=0?C.green:C.text }}>
                {remaining-+amt<=0?"🎉 Fully settled!":fmt(remaining-+amt)+" remaining"}
              </strong>
            </p>
          )}
        </div>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <Btn variant="outline" onClick={onClose}>Cancel</Btn>
          <Btn onClick={save} style={{ opacity:+amt>0?1:0.4, background:`linear-gradient(135deg,${color},${color}bb)`, boxShadow:"none" }}>
            Log payment
          </Btn>
        </div>
      </div>
...
function UtangScreen({ utangs, setUtangs }) {
  ...
  const [sheet,       setSheet]       = useState(null);
  const [paySheet,    setPaySheet]    = useState(null); // utang obj for payment
  const [confirm,     setConfirm]     = useState(null);
  const [expanded,    setExpanded]    = useState({}); // id -> bool for payment history
  ...
  const logPayment = (utangId, payment) => {
    setUtangs(prev=>prev.map(u=>{
      ...
      const settled   = totalPaid >= u.amount;
      return { ...u, payments, settled };
    }));
    setPaySheet(null);
  };

  // Compute remaining per utang
  const remaining = u => u.amount - (u.payments||[]).reduce((s,p)=>s+p.amount,0);
    ...
    <div className="screen-wrap" style={{ padding:"22px 18px 16px", display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={{ display:"flex", background:C.surface, borderRadius:12, padding:4, border:`1px solid ${C.border}` }}>
        {[["all","All"],["iowe","I Owe"],["theyowe","They Owe"]].map(([v,l])=>(
          <button key={v} onClick={()=>setView(v)} className="tap-btn" style={{ flex:1, padding:"8px 4px", borderRadius:9, border:"none", cursor:"pointer", background:view===v?C.card:"none", color:view===v?C.text:C.textSub, fontSize:12, fontWeight:700, fontFamily:"DM Sans,sans-serif", transition:"all 0.18s" }}>{l}</button>
        ))}
      </div>
      ...
      {filtered.map((u,i)=>{
        ...
        return (
          <Card key={u.id} animDelay={i*40}
            style={{ opacity:u.settled?0.65:1, border:`1.5px solid ${u.settled?C.border:color+"40"}` }}>

            {/* Header row */}
            <div style={{ display:"flex", alignItems:"flex-start", gap:12, marginBottom:12 }}>
              <div style={{ width:44, height:44, borderRadius:14, background:u.direction==="iowe"?`${C.coral}15`:`${C.green}12`,
            ...
            {/* Progress bar (only when there are payments) */}
  ...
  return (
    <BottomSheet onClose={onClose} title={utang?"Edit utang":"Log utang"}>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div>
          <SLabel>Who?</SLabel>
          <Inp autoFocus value={person} onChange={setPerson} placeholder="e.g. Mico, Jessa, Sir JA…"/>
        </div>
        ...
        <div>
          ...
          <Inp value={note} onChange={setNote} placeholder="e.g. lunch, GCash load, taxi share…"/>
        </div>
